...
function initPara() {
    ...
    var fieldFlag="true";
    var fieldData= Base64.decode($('#govFieldJson').val());
    if(typeof(fieldData)!="undefined" && fieldData!='{"fieldData":]}'){
        ...
        if(fieldDataObjeLength>0){
            $.each(fieldDataObj.fieldData,function(i,item){
                ...
                if($("*[name='"+fieldCodeData+"']").length>0 && $("*[name='"+fieldCodeData+"']")[0].value==""){
                    fieldFlag="false";
                    whir_alert(item.fieldNameData+comm.v_notempty,function(){$("*[name='"+fieldCodeData+"']").focus()});
                    return false;
                }
            });
        }
    ...
    var tempFlagDZ="";
	var optLength=0;
    if($("input[name='sendFileDepartWord']").length > 0 && $("input[name='sendFileDepartWord']")[0].value != "") {
        tempFlagDZ="false";
        var tempjfdzValue=$("input[name='sendFileDepartWord']")[0].value;
		if($("select[id='sendFileDepartWordTest']").length>0){
		 optLength = $("select[id='sendFileDepartWordTest'] option").length;
		}
       
        if(optLength>0){
            $("select[id='sendFileDepartWordTest'] option").each(function(){
                if($(this).val() == tempjfdzValue){
                    tempFlagDZ="true";
                }
            });
        }
    }
    if(tempFlagDZ!=""&&tempFlagDZ=="false"&&optLength>0){
        whir_alert('机关代字不存在！',function(){$("#sendFileDepartWordTest").focus()});
        return false;
    }
	/*  if($("input[name='documentSendFileTitle']")[0].value==""){
	  ...
	  if($("input[name='documentSendFileTitle']")[0].value.indexOf("#")>=0){
		  whir_alert(comm.govbiaotibunenghan+"'#'",function(){$("input[name='documentSendFileTitle']").focus()});
		  return false;
	  }
	   if($("input[name='documentSendFileTitle']")[0].value.indexOf("&")>=0){
		  whir_alert(comm.govbiaotibunenghan+"'&'",function(){$("input[name='documentSendFileTitle']").focus()});
...
//alert(numxuhao);
	  if(numId!=""&&numxuhao!=""){
			...
			if (!http_request) { /* 异常，创建对象实例失败*/
				window.alert(comm.govbccjxmlsldx);
				return false;
			}else{
			...
			if (http_request.readyState == 4) { /* 判断对象状态*/
				if (http_request.status == 200) { /*信息已经成功返回，开始处理信息*/
					...
					//alert("aaa"+result);
					if(result == 0){
						whir_alert(comm.govwenhaochongfu);
						return false;
					}
				} else { /*页面不正常*/
					alert(comm.govnsqqdymyyc);
				}
			}
	   }
...
function cmdSaveDraft(){
    ...
    $("#dataForm").ajaxSubmit({
        type: "post",
        url: "/defaultroot/GovDocSendProcess!saveDraft.action",

        success: function(result){
            //返回提示信息
            var tempData=eval("("+result+")");
            var tempRecordId=tempData.data.recordIdTemp;
            $("#p_wf_recordId").val(tempRecordId);
            saveDraftOk();
        },
...
function saveDraftOk(){
	whir_alert(comm.savesucc,function(){
...
//alert(1);
		$("#sendToMyOtherForm").ajaxForm({
		     success: function(responseText){ 
			     popup({id:'cmdSendToMyRange_pop',title:comm.govbufa,fixed: true, max:true, width:'300',left: '50%', top: '50%', padding: 0,drag: true, resize: false,lock: true,content: responseText}); 		 
...
//		 width:'590px',height:'430px', drag: true, resize: true,lock: true,content: "url:"+url}); 
	...
	//document.sendToMyForm.submit();
		...
		$("#sendToMyForm").ajaxForm({
		     success: function(responseText){ 
				// alert(responseText);
			       popup({id:'cmdSendToMyRange_show_pop',title: comm.govfenfafanwei,fixed: false,max:true,
...
function cmdSendclose(){
	...
	if($("input[name='p_wf_pool_processType']")[0].value == "1"){
		...
		$.dialog({id:'SendcloseWinId',title:'分发',width:600,height:400,content: 'url:'+hhref+''});
...
//< %=sendStatus% >
	...
	if( t_p_wf_recordId == "" ){
		//alert(11);
		whir_alert(comm.govqxbczff);
	    return;
	}
	//alert(12);
	
	if(toName==""){
	    whir_alert(comm.govqxzjsz);
	    return;
	}
//alert(13);
	...
	whir_alert(comm.govfenfachenggong);
...
function cmdChangeNumber(){
	...
	var  wordValue=document.getElementsByName("sendFileDepartWord")[0].value;//$("*[name='sendFileDepartWord']").val(); 
     ...
     if(wordValue!="" && wordValue != undefined  && wordValue!=null){
             mystr=wordValue.split(";");      
       		 var  sendWordId=mystr[0];
			
	   if(sendWordId==""){
			whir_alert(comm.govqxxzjgdz);
			return ;

       }else{
			  //openPupWin("/defaultroot/GovSendFileAction.do?action=initZjkySendWord&sendWordId="+sendWordId+"&changeNumType=add",400,200);
	//openPupWin("/defaultroot/GovSendFileAction.do?action=initZjkySendWord&sendWordId="+sendWordId+"&changeNumType=add",400,200);
   				popup({id:'cmdChangeNumber_pop',title: comm.govbianhao,fixed: true, left: '50%', top: '50%',width:'650px',height:'180px',padding: 0,drag: true, resize: true,lock: true,content: 'url:GovDocSend!initZjkySendWord.action?sendWordId='+sendWordId+'&changeNumType=add'});
		   }
...
function cmdPrintTemp(){
    //打印
    //打印
    var url='/defaultroot/modules/govoffice/gov_documentmanager/choosedocumentTemplate.jsp?tempRecordID='+$("*[name='p_wf_recordId']").val()+'&govFormID='+$("*[name='p_wf_tableId']").val();
    openWin({url:url,width:600,height:500,isFull:false,winName:'printWindow'});
...
function cmdGovUnionTask(){
    ...
    var url='/defaultroot/cbdb!addorUpdateTaskJSP.action?goldgridId='+$("input[name='content']")[0].value+'&acceName='+acceName+'&acceSaveName='+ acceSaveName +'&sfzrw=0&rwlyid=&taskName=' + ($("*[name='documentSendFileTitle']").val()) ;

    //var url='/defaultroot/cbdb!addorUpdateTaskJSP.action?goldgridId='+$("input[name='content']")[0].value+'&acceName='+acceName+'&acceSaveName='+ acceSaveName +'&sfzrw=0&rwlyid=&taskName=' + $("*[name='receiveFileTitle']").val();
    openWin({url:url,width:1050,height:620,winName:'cmdGovUnionTaskWin'});
}
