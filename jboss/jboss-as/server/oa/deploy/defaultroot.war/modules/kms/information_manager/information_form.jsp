...
String oldchannel=request.getParameter("oldchannel")==null?"":request.getParameter("oldchannel").toString();
//20160728 -by jqq 信息保存草稿在公文同步信息的时候不展示
String isfromgov = request.getParameter("isfromgov")==null?"":request.getParameter("isfromgov").toString();
//String curModifyField=request.getAttribute("curModifyField")==null?"":request.getAttribute("curModifyField").toString();
%>
<input type="hidden" id="isyiboflag" name="isyiboflag" value="<%=_isyiboflag%>"/>
<input type="hidden" id="oldchannel" name="oldchannel" value="<%=oldchannel%>"/>
	...
	<table width="100%" border="0" cellpadding="2" cellspacing="0" class="Table_bottomline">
		...
		<tr>
			...
			<s:if test="#request.curModifyField==null || #request.curModifyField.indexOf('$information.informationTitle$') > -1">
				...
				<s:if test="#request.action=='start' || #request.action=='restart'">
					<s:if test="#request.curModifyField==null || #request.curModifyField.indexOf('$information.mobileTop$') > -1">
						<s:checkboxlist name="information.mobileTop" list="#{'1':'移动头条'}"/>
					</s:if>
					<s:else>
						<s:checkboxlist name="information.mobileTop" list="#{'1':'移动头条'}" disabled="true"/>
						<s:hidden id="mobileTop" name="information.mobileTop"/>
					</s:else>
				</s:if>
				</span>
			</s:if>
		...
		<tr id="info_add_tr1">
            ...
            %>
			<%
			if((wordEidt && !isCOSClient) || (isLinux && "1".equals(isfromgov))){%>
		...
		<tr id="selectImg">
			...
			<td>
				...
				<jsp:include page="/public/upload/uploadify/upload_include.jsp" flush="true">
				   <jsp:param name="thumbnail" value="800x999999_small;" />
				...
				<s:else>
					...
					<s:hidden id="displayImage" name="information.displayImage"/>
				</s:else>
				</span>
				</div>
			</td>
		...
		<tr id="edit">
			...
			<s:if test="#request.curModifyField==null || #request.curModifyField.indexOf('$information.informationContent$') > -1">
				...
				<iframe id="newedit" src="<%=rootPath%>/public/edit/ewebeditor.htm?id=information.informationContent&style=coolblue&lang=<%=session.getAttribute("org.apache.struts.action.LOCALE")%>&hiddenPreviewTab=1" frameborder="0" scrolling="no" width="100%" height="350"></iframe>
			</s:if>
			<s:else>
				<s:hidden name="information.informationContent" id="informationContent"/>
				<iframe id="newedit" src="<%=rootPath%>/public/edit/ewebeditor.htm?id=information.informationContent&style=coolblue&lang=<%=session.getAttribute("org.apache.struts.action.LOCALE")%>&readonly=1&hiddenPreviewTab=1" frameborder="0" scrolling="no" width="100%" height="350"></iframe>
			</s:else>
			</td>
		</tr>
		
		<input type="hidden" id="content" name="content" value="<%=request.getAttribute("content")!=null?com.whir.component.security.crypto.EncryptUtil.replaceHtmlcode(request.getAttribute("content").toString()):""%>">
		...
		<s:if test="#request.action=='add' || #request.action=='load' || #request.action=='loaddraft'">
		<tr>
			...
			<td nowrap>
				...
				<%if(!"1".equals(isfromgov)){%>
				<input type="button" class="btnButton4font" onClick="saveInfoDraft(0,this);" value="<s:text name='info.savedraft'/>" />
				<%}%>
				</s:if>
				<s:elseif test="#request.action=='loaddraft'">
				<%if(!"1".equals(isfromgov)){%>
				<input type="button" class="btnButton4font" onClick="saveInfoDraft(0,this);" value="<s:text name='info.savedraft'/>" />
				<%}%>
				</s:elseif>
				<input id="preview-button" type="button" class="btnButton4font" onClick="infoPreView();" value="<s:text name='info.Preview2'/>"/>
...
<div id="docinfo1" style="display:none;">
	<table width="100%" border="0" cellpadding="2" cellspacing="0" class="Table_bottomline">
		<tr>  
			<td for="<s:text name='info.newinfosecondtitle'/>" width="8%" class="td_lefttitle" nowrap><s:text name="info.newinfosecondtitle"/>：</td>
			<td width="42%">
			...
			<td for="<s:text name='info.newinfoarticleno'/>" width="8%" class="td_lefttitle" nowrap><s:text name="info.newinfoarticleno"/>：</td>
			<td width="42%">
		...
		<tr>
			...
			<td width="8%" class="td_lefttitle" nowrap><s:text name="info.publictag"/>：</td>
			<td width="42%">
			...
			<s:else>
				<s:textfield type="text" class="inputText" style="width:93%" readonly="true" id="choosed_tagName" name="information.informationKey"  whir-options="'promptText':'<s:text name='info.selecttag'/>'"/>
			</s:else>
			<%--<s:if test="#request.curModifyField==null || #request.curModifyField.indexOf('$information.informationKey$') > -1">
		...
		<tr>
			<td for="<s:text name='info.viewauthor'/>" width="8%" class="td_lefttitle" nowrap><s:text name="info.viewauthor"/>：</td>
			<td width="42%">
			...
			<td for="<s:text name='info.authorstatauthordep'/>" width="8%" class="td_lefttitle" nowrap><s:text name="info.authorstatauthordep"/>：</td>
			<td width="42%">  
		...
		<tr>
			...
			<td for="<s:text name='info.newinfosource'/>" width="8%" class="td_lefttitle" nowrap>
		...
		<tr>
			...
			<td width="42%" nowrap>
				...
				<s:hidden id="informationReaderId_" name="informationReaderId_" value="%{#request.informationReaderId_}"/>
		...
		<tr>
			...
			<td colspan="3" nowrap>
				...
				<input id="preview-button2" type="button" class="btnButton4font" onClick="infoPreView();" value="<s:text name='info.Preview2'/>" />
				<input type="button" class="btnButton4font" onClick="resetDataForm(this);" value="<s:text name='comm.reset'/>" />
				<input type="button" name="exit" class="btnButton4font" onClick="closeWindow(null);" value="<s:text name='comm.exit'/>" />
			</td>  
		</tr>
