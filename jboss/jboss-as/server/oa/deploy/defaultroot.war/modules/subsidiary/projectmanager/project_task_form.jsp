...
<s:hidden id="startDate" value="%{#request.startDate}" />
<!-- "添加子任务"时，带入的ID -->
...
<table width="100%" border="0" cellpadding="2" cellspacing="0" class="Table_bottomline">
    ...
    <tr>    
        ...
        <td>    
            ...
            <s:if test="taskId != null">
                <s:hidden id="tmp_preTaskId" value="%{taskPO.preTaskId}" /> 
                <s:hidden id="preTaskEndDate"  value="%{#request.preTaskEndDate}"  /> 				
            </s:if>
            <s:if test="%{#readType != 2}">
			<select name="taskPO.preTaskId" id="preTaskId" class="easyui-combobox" style="width:290px;" data-options="selectOnFocus:true,onSelect: function(record){selectPreTask(record);}, editable:false" >
                <s:iterator var="obj" value="#request.preTaskInfoList" >
                    <option value="<s:property value='#obj[0]'/>" start="<s:date name='#obj[2]' format='yyyy-MM-dd' />" end="<s:date name='#obj[3]' format='yyyy-MM-dd' />"><s:property value='#obj[1]' escape="false"/></option>
                </s:iterator>
            </select>
			</s:if> 
            <s:else>
			<select name="taskPO.preTaskId" id="preTaskId" class="easyui-combobox" style="width:290px;" data-options="selectOnFocus:true,onSelect: function(record){selectPreTask(record);}, editable:false" disabled="disabled">
                <s:iterator var="obj" value="#request.preTaskInfoList" >
                    <option value="<s:property value='#obj[0]'/>" start="<s:date name='#obj[2]' format='yyyy-MM-dd' />" end="<s:date name='#obj[3]' format='yyyy-MM-dd' />"><s:property value='#obj[1]' escape="false"/></option>
                </s:iterator>
            </select>		
    ...
    <tr>    
        ...
        <td for="实际开始日期" class="td_lefttitle" nowrap>实际开始日期：</td>    
        <td>
            <s:if test="%{#readType != 2}">   
            <s:textfield name="taskPO.ReaLBeginTime" id="ReaLBeginTime" cssClass="Wdate whir_datebox" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true,maxDate:'#F{$dp.$D(\\'ReaLReaLEndTime\\',{d:0})}',isShowClear:false})"  readonly="true">
            ...
            至 
            <s:if test="%{#readType != 2}">   
            <s:textfield name="taskPO.ReaLReaLEndTime" id="ReaLReaLEndTime" cssClass="Wdate whir_datebox" onClick="WdatePicker({dateFmt:'yyyy-MM-dd',readOnly:true,minDate:'#F{$dp.$D(\\'ReaLBeginTime\\',{d:0})}',isShowClear:false})"  readonly="true">
                <s:param name="value"><s:date name="taskPO.ReaLReaLEndTime" format="yyyy-MM-dd" /></s:param>
            </s:textfield> 
             </s:if> 
            <s:else> 
            <s:textfield name="taskPO.ReaLReaLEndTime" id="ReaLReaLEndTime" cssClass="Wdate whir_datebox"  readonly="true">
                <s:param name="value"><s:date name="taskPO.ReaLReaLEndTime" format="yyyy-MM-dd" /></s:param>
            </s:textfield> 
            </s:else> 
        </td>     
