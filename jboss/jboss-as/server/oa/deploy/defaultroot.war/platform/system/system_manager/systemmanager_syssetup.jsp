...
String forumorder=(String)sysSetupMap.get("oa_forumorder");
...
String writeRangeIds = (String)sysSetupMap.get("writeRangeIds");
String writeRangeNames = (String)sysSetupMap.get("writeRangeNames");
if(options.charAt(1)!='1'){//手写意见不使用
	//writeRangeIds="";
	//writeRangeNames="";
}
System.out.println("===============================writeRangeIds:"+writeRangeIds);
String ydkq = (String)sysSetupMap.get("ydkq");
System.out.println("===============================ydkq:"+ydkq);
...
<div class="BodyMargin_10" style="padding-top:5px;">
    <div class="docBoxNoPanel clearfix" style="min-width:800px; overflow-x:auto;" >
        ...
        <table class="Table_bottomline" border="0" cellpadding="0" cellspacing="0">
            ...
            <tr>
                ...
                <td>
                    ...
                    <input type="radio" value="2" id="captcha2" name="captcha" <%if("2".equals(captcha)){%>checked<%}%>>连续输入错误时
                </td>
            </tr>
			<tr style="display:">
			...
			<tr>
			 ...
			 <tr>
                ...
                <th>手写意见：</th>
                <td>
                    <input type="radio" value="0" id="sign0" name="sign" checked  onclick="showWriteRange(0);" >不使用&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="radio" value="1" id="sign1" name="sign" <%if(options.charAt(1)=='1'){%>checked<%}%>  onclick="showWriteRange(1);">使用
					<div  id="writeRangeDIV"  <%if(options.charAt(1)!='1'){%> style="display:none"<%}%> cssStyle="width:100%" >
					
						<input style="display:none" name="writeRangeIds" id="writeRangeIds" value='<%=writeRangeIds%>'/>
                        范围： <textarea name="writeRangeNames" id="writeRangeNames"  cssStyle="width:90%" readonly="true"  value='<%=writeRangeNames%>' title='<%=writeRangeNames%>'><%=writeRangeNames%></textarea>
						<a class="selectIco textareaIco" onclick="selectWriteRange();"></a>
					</div>
                </td>
            </tr>
            <tr>
			...
			<tr >
                ...
                <td>
					...
					<input type="checkbox" value="<%=location%>" id="location" name="location" <%if("1".equals(location)){%>checked<%}%> 
                     onclick="OAlocation(this)" style="display:none" />客户端定位&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="checkbox" value="<%=wxlocation%>" id="wxlocation" name="wxlocation"  <%if("1".equals(wxlocation)){%>checked<%}%>
                     onclick="wxlocationaction(this)" style="display:none"/>微信定位
					 -->
					 
					<input type="radio" value="0" id="ydkq1" name="ydkq"  <%if("0".equals(ydkq)){%>checked<%}%>>不使用&nbsp;&nbsp;&nbsp;&nbsp;
                    <input type="radio" value="1" id="ydkq2" name="ydkq" <%if("1".equals(ydkq)){%>checked<%}%>>使用
                </td>
            </tr>
			...
			<tr>
                ...
                <th>工作流引擎：</th>
                <td>
                    <input type="radio" value="0" id="workflowType0" name="workflowType" <%if("0".equals(workflowType)){%>checked<%}%>>ezFLOW&nbsp;
                    <input type="radio" value="1" id="workflowType1" name="workflowType" <%if("1".equals(workflowType)){%>checked<%}%>>老引擎
			...
			<tr>
                ...
                <th>信息列表排序：</th>
                <td>
                    <input type="radio" value="0" id="infoorder1" name="infoorder"  <%if("0".equals(infoorder)){%>checked<%}%>>发布日期&nbsp;
                    <input type="radio" value="1" id="infoorder2" name="infoorder"  <%if("1".equals(infoorder)){%>checked<%}%>>修改日期&nbsp;&nbsp;&nbsp;
			 ...
			 <tr>
                ...
                <td>
                    ...
                    <input type="radio" value="0" id="mailRemindType1" name="mailRemindType" onclick="changeMailRemidType('0')" <%=(!"1".equals(mailRemindType[0]) && !"2".equals(mailRemindType[0]))? "checked": ""%>>内部邮件
					 <input type="radio" value="-1" id="mailRemindType2" name="mailRemindType" onclick="changeMailRemidType('-1')" <%=("0".equals(mailremind) )? "checked": ""%>>无
                </td>
            </tr>
		...
		<div style="float:right; width:55%">
			...
			<tr style="display:none">
				<th >登录超时时间：</th>
				<td> 
				<input type="text" id="logintimelimit" name="logintimelimit" class="inputText" style="width:40px" value="10" maxlength="3">&nbsp;分钟<br/>
				<font>&nbsp; </font>
				</td>
			 </tr>
 			<tr>
			...
			<tr>
                ...
                <th>论坛列表排序：</th>
                <td>
                    <input type="radio" value="0" id="forumorder1" name="forumorder" <%if("0".equals(forumorder)){%>checked<%}%>>发表时间
                    <input type="radio" value="1" id="forumorder2" name="forumorder" <%if("1".equals(forumorder)){%>checked<%}%>>回帖时间
			...
			<!--<tr id="historyMail_tr" style="display:none">  <th colspan="2" style="text-align: center; color: red; "> </th>   </tr> -->
			<tr> 
				<th>&nbsp; </th>
                <td style="  color: red;  padding-left: 5px">历史邮件只能查看，不能进行回复、转发等操作。 </td>
                
            </tr>
           <tr id="mailSMTP_span1" style="display:none">
                ...
                <td valign="top">
                    <div style="padding-left:20px;display:inline;">
                        <table border="0" cellpadding="0" cellspacing="0" style="margin-top:-20px;">
							<tr >
								 <th>电子邮件地址：</th>
								 <td><input class="inputText" type="text" id="mailAddr" name="mailAddr" maxlength="50" value="<%=mailRemindType[3]%>" style="width:120px;">
								 </td>
							</tr>
...
function save() {
 ...
 //alert(myselect.options[index].text);
    ...
    if (isNaN($('#wordlimitsize').val()) || parseInt($('#wordlimitsize').val()) < 0 || $.trim($('#wordlimitsize').val())=='') {
        ...
        $('#wordlimitsize').focus();
        return;
    }else if (parseInt($('#wordlimitsize').val()) == 0) {
...
function changeMailRemidType(val) {
    if (val == '1' || val == '2') {
        $('#mailSMTP_span').css('display', '');
		 $('#mailSMTP_span1').css('display', '');
    } else {
        $('#mailSMTP_span').css('display', 'none');
		 $('#mailSMTP_span1').css('display', 'none');
    }
}
function changeMailRemid(val) {
    if (val == '0' ) {
        $('#mailSMTP_span').css('display', 'none');
		 $('#mailSMTP_span1').css('display', 'none');
    } else {
...
function showWriteRange(i) {
    if (i==0) {
        $('#writeRangeDIV').css('display', 'none');
		//$('#writeRangeNames').attr("value",'');
		//$('#writeRangeIds').attr("value",'');
		//$('#writeRangeNames').text('');
    } else {
        $('#writeRangeDIV').css('display', '');
    }
}

function selectWriteRange(){
	openSelect({allowId:'writeRangeIds', allowName:'writeRangeNames', select:'user', single:'no', show:'userorggroup', range:'*0*'});
}
