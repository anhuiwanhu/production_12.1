...
function setAbsolute(obj) {
    ...
    try {
        ...
        /*if($("#addRow").val()=="false" && $("#p_wf_recordId").val()!=""){
        	div.style.width="15px";
        	if($("#delRow").val()=="false" && $("#p_wf_recordId").val()!=""){
            	//div.style.width="0px";
        		div.style.visibility = "hidden";
            }
        }else{
...
function addRow(ind, isInvoke) {
	if($("#addRow_").val()=="false" && $("#p_wf_recordId").val()!=""){
		whir_alert("您没有新增行权限，若想新增数据请联系管理员！", null);
		return;
	}
    ...
    var form_p_wf_cur_ModifyField = "";//当前可以修改的字段
    try{
    ...
    try {
		...
		newTr.setAttribute("data","newData")//为删除数据准备
        ...
        for (var i = 0, k=row.cells.length; i < k; i++) {
            ...
            $(_htmlObj).find("input[type=hidden][name$=_showtype]").each(function(n){
                ...
                _showtype_ = thisVal;//字段类型  （单行文本 多行文本……）

                _field_name = thisName;//字段名称 （whir$t3201_f3102 ……）
                ...
                if(_showtype_=='103'||//单选
                    ...
                    var _new_ran_ = EZFORM.getRandomNum();// _rnd_7001
					_new_ran_1 = _new_ran_;
                    var _old_ran_ = document.getElementsByName(PREFIX_NEW_COMPONENT+thisName)[_index].value;
					//new_component_whir$t3201_f3102      _rnd_065891997109425
                    while(_html.indexOf(_old_ran_)!=-1){
                        _html = _html.replace(_old_ran_, _new_ran_);
                    }

                    if(_showtype_=='115'){
                        _new_name = thisName + _new_ran_ + "_fileName";//whir$t3201_f3102_rnd_7001_fileName
                        _new_id = thisName + _new_ran_ + "_saveName";//whir$t3201_f3102_rnd_7001_saveName
                        ...
                        _showtype_=='704'||
                        _showtype_=='705'){
                        _new_name = thisName + _new_ran_ + "_Name";//whir$t3201_f3102_rnd_7001_Name
                        _new_id = thisName + _new_ran_ + "_Id";//whir$t3201_f3102_rnd_7001_Id
						_new_value_name = document.getElementsByName(thisName+_old_ran_+"_Name")[0].value;
						_new_value_id = document.getElementsByName(thisName+_old_ran_+"_Id")[0].value;
                    }else{
                        _new_name = thisName + _new_ran_;//whir$t3201_f3102_rnd_7001
						//_new_value_name = document.getElementsByName(thisName+_old_ran_)[0].value;
						//_new_value_id = document.getElementsByName(thisName+_old_ran_)[0].value;
                    }
                }else if(_showtype_ == '203'){//计算字段
                ...
                }else if(_showtype_ == '801' || _showtype_=='802'){
					_new_value = document.getElementsByName(thisName)[_index].value;
					_new_txt = document.getElementsByName(thisName+"_txt")[_index].value;
				}else{
					_new_value = document.getElementsByName(thisName)[_index].value;
				}
            });
            ...
            if(_showtype_=='116'||
                ...
                document.getElementsByName(_field_name)[ri].value = _autocode_;
                        ...
                        _showtype_=='704'||
                        _showtype_=='705'){
				document.getElementsByName(_field_name+_new_ran_1+"_Name")[0].value=_new_value_name;
				document.getElementsByName(_field_name+_new_ran_1+"_Id")[0].value=_new_value_id;
			}else if(_showtype_=='801' || _showtype_=='802'){
				document.getElementsByName(_field_name)[ri].value=_new_value;
				document.getElementsByName(_field_name+"_txt")[ri].value=_new_txt;
			}else if(_showtype_!='115' && _showtype_!='103' && _showtype_!='104'){
				document.getElementsByName(_field_name)[ri].value=_new_value;
			}
            ...
            /*if(_new_name!=''){
                ...
                if(form_p_wf_cur_ModifyField.indexOf('$'+_field_name+'$')==-1 || $('#addNull_').val()=="true"){//不可编辑或者在流程设置中设置了新增行数据为空
                    document.getElementsByName(_field_name)[ri].value = '';
                    try{
						if(_showtype_=='801' || _showtype_=='802'){//预算部门 预算科目数据置空
							document.getElementsByName(_field_name+"_txt")[ri].value = '';
							document.getElementsByName(_field_name)[ri].value = '';
						}else if(_showtype_=='210'||
								_showtype_=='211'||
								_showtype_=='212'||
								...
								_showtype_=='704'||
								_showtype_=='705'){
							document.getElementsByName(_field_name+_new_ran_1+"_Name")[0].value='';
		...
		//项目需求  子表数据自动回填
		/*var targetField = document.getElementsByName("whir$t3305_f4399");
		var userAgent = navigator.userAgent; //取得浏览器的userAgent字符串
		var isOpera = userAgent.indexOf("Opera") > -1;
		if (userAgent.indexOf("compatible") > -1 && userAgent.indexOf("MSIE") > -1 && !isOpera) {//IE
			targetField[ri].attachEvent("onkeyup",function(){queryData_more(ri);});
		}else{
			targetField[ri].addEventListener("keyup",function(){queryData_more(ri);});
		}*/
...
//删除行
function delRow() {
	var data = currentRow.getAttribute("data");
	if(data==null || data=="" ){
		if($("#delRow_").val()=="false" && $("#p_wf_recordId").val()!=""){
			whir_alert("您没有删除已有行权限，若想删除已有行数据请联系管理员！", null);
			return;
		}
	}
...
function checkOnlyField_(val, flag){
    ...
    if(_p_wf_openType == 'startAgain' || _p_wf_openType == 'reStart'){//再次发送  重新发送
        infoId = '';
    }
...
function checkMustWrite(){
        ...
        if(obj){
            ...
            if (obj.length) {
                for (var k = 0, len=obj.length; k < len; k++) {
                	var spanAll = $(obj[k]).nextAll("span");
                	if(spanAll!=null && spanAll!=undefined && ""!=spanAll){
                		var classCss = "";
	                	for(var m=0;m<spanAll.length;m++){
	                		classCss+=$(spanAll[m]).attr("class");
	                	}
	                	if(classCss.indexOf("mustFillSpan")==-1){
	                		continue;
	                	}
                	}else{
                		continue;
	...
	//20161104
	if($("input[type=hidden][name$=_showtype][value=115]").length>0){
	...
	//20170609
	if($("input[type=hidden][name$=_showtype][value=216]").length>0){
		$("input[type=hidden][name$=_showtype][value=216]").each(function(n){
			var _name = $(this).attr('name');
			var _fieldName = _name.substring(0, _name.length-9);
			var _new_component_ = document.getElementsByName('new_component_'+_fieldName)[0].value;
			var ids = document.getElementById(_fieldName + _new_component_ + '_Id').value;
			...
			if (_temp.length > parseInt(_size)) {
				canSubmit = false;
				whir_alert("多选组织和人员字段长度超出限制，请重新选择！");            
				return false;
			}
		});
	}
...
function initSubtableImport(){
    ...
    for(var i=0; i<subTable.length; i++){
        ...
        if(div_){
            ...
            $("#"+replaceDollar(val)+"DIV").prepend("<div id="+val+"_subtable align=right style='width:192px;border:0px solid black;margin-bottom:3px;'><a style='cursor:pointer' onclick='javascript:downloadSubtableTemplate(\""+val+"\", \"\")' target='_blank'>下载导入模板</a>&nbsp;<input type='button' onclick='impSubData(\""+val+"\", \"\")' title='导入文件的数据列必须与子表数据列一一对应' class='btnButton4font' value='导&nbsp;&nbsp;&nbsp;入' style='vertical-align:middle'/></div>");
        }
    }
}
