        ...
        for(var i = 0; i < _len_; i ++){
            ...
            if(_computeField_){
                ...
                if(__cfv__!=''){
					...
					var isInteger=false;
					try{
						var _size = $(document.getElementsByName($(_computeField_).attr('name')+'_type')[0]).attr('size');//document.getElementsByName($(obj).attr('name')+'_size')[0].value;
						if (_size == '9') {
							isInteger=true; 
						}
					}catch(e){}
					...
					if(isInteger){ 
						__cfv__ = parseFloat(__cfv__).toFixed(0); 
					}else{
