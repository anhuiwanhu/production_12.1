...
<%
    ...
    String noNeedfsjx=request.getParameter("noNeedfsjx")==null?"":request.getParameter("noNeedfsjx")+"";
	//下一办理活动
	...
	//内部邮件提醒
	String haveMail ="0";
	haveMail =sysRed.getOa_mailremind(session.getAttribute("domainId").toString());
...
<style>
	.a_select {color:blue}
	.div_select {background-color: #ededed}
</style>
<body>
<!-- 分叉活动选择时 记住 选择的是哪个活动-->
...
<div >
	<table width="586px"  id="wf_c_mainTable" border="0" cellspacing="0" cellpadding="0" class="Table_bottomline" >
		<tr class="Table_nobttomline">
			<td valign="middle"  class="td_lefttitle" width="85px" style="text-align:left;"><bean:message bundle="filetransact" key="file.nextactivity"/><span class="MustFillColor">*</span>：</td>
			<td width="501px">&nbsp;</td>
		</tr>

		<tr>
			<td colspan="2" width="100%" class="td_lefttitle">
			<%if(nextActivitys !=null && nextActivitys.size() >3){%>
				<div class="newxLink"  style="max-height:100px;width:586px;margin-right:7px;text-align:left;">
			<%}else{%>
				<div style="max-height:100px;text-align:left;">
			<%}%>
					<logic:iterate id="navo" name="nextActivitys" indexId="indexid"> 
					<%   				  
					avo = (ChoosedActivityVO)navo; 

					int _index = indexid + 1;
					...
					if(dealTransactorInfoVO!=null){
						_scopeType=dealTransactorInfoVO.getType();
					}
					%>
					<!--此处不可以乱加HTML标签-->
					<div <%=li_name_display%> <%if(_index ==1){%>class="div_select"<%}%>>
						<label><input type="<%=inputType%>" name="chooseActivity"  <%=checkreturn%>   <%=radioAttributeStr%>  <%=checkedStr%> />
						<input type="hidden" name="selected_scopeType" id="selected_scopeType" value="<%=_scopeType%>">
						<input type="hidden" name="selected_activityId" id="selected_activityId" value="<%=avo.getActivityId()%>"></label>
						<label><a href="#one" <%if(_index ==1){%>class="a_select"<%}%> onclick="checkOnclikFun(this,'<%=avo.getActivityId()%>',5,'<%=inputType%>')"><%=getActivityName+isMainOrSon %></a></label>
						<label id="selectedUser" style="color:blue"></label>
					</div>
					</logic:iterate>
				</div>
			</td>
		...
		<tr class="Table_nobttomline"   id="remindTr">
			<td valign="top" class="td_lefttitle" style="text-align:left;width:85px;"><%=Resource.getValue(local,"workflow","workflow.remindersettings")%>：</td>
			<td style="text-align:left;">
				<%if(haveMail.equals("1")){%>
				...
				<%}if(showSmsRemind){%>
				<span id="temp_sendNeedNoteType_span_<%=eachActivityId%>"  style="display:none"><input type="checkbox" name="temp_sendNeedNoteType_<%=eachActivityId%>" id="temp_sendNeedNoteType_<%=eachActivityId%>"  value="1" onClick="showSmsContentHref('<%=eachActivityId%>')"><bean:message bundle="workflow" key="workflow.activitysmsremind"/>&nbsp;</span>
				<span id="showNoteDiv_a_<%=eachActivityId%>"  style="display:none" ><a  href="#" onClick="showSmsContent();"><%=Resource.getValue(local,"filetransact","file.smsremindcontent")%></a>&nbsp;&nbsp;</span>
				<%}%>
		...
		<tr class="Table_nobttomline" id="tipsmstr" style="display:none">
			<td colspan="2"> 
				<div id="smsContentTR" style="padding: 7px;display:none">
					<textarea name="temp_smsContent"  id="temp_smsContent" wrap="physical"   class="inputTextarea" style="width:100%;height:60px"><%=request.getAttribute("smsContent")==null?"":request.getAttribute("smsContent").toString()%></textarea>
				</div>
			</td>
		</tr>
		
		<tr class="Table_nobttomline" <%if(isEndActivity){%>style="display:none"<%}%>>
			<td colspan="2">
				<div id="dealTipTr" style="padding: 7px;">
					<!--<textarea name="temp_dealTips"  wrap="physical"  id="temp_dealTips"   class="inputTextarea" style="width:100%;height:60px"></textarea>-->
					<input type="text" name="temp_dealTips" id="temp_dealTips" class="inputText" whir-options="'promptText':'办理提示'" style="width:100%" />
				</div>
			</td>
		</tr>
		...
		<tr class="Table_nobttomline">
			<td colspan="2" style="text-align:left;">
				<div style="padding-left: 5px;padding-right: 7px;">
					...
					<%if(!"1".equals(noNeedfsjx)&&"startOpen".equals(p_wf_openType)){%>
...
function  checkOnclikFun(obj,onclik_aid,ctype,inputType){
	if(inputType =='checkbox'){
		var dealType='_deal_';
		var selectedUserHtml ='';
		var selected_scopeType ='';
		var selected_activityId ='';
		$(obj).parent().parent().parent().find('a').each(function(){
			var styleValue =$(this).attr('class');
			if(styleValue =='a_select'){
				//获取选人类型
				selected_scopeType =$(this).parent().parent().find('input[name=selected_scopeType]').val();
				//获取活动ID
				selected_activityId =$(this).parent().parent().find('input[name=selected_activityId]').val();
			}
		});
		//alert(selected_scopeType);
		//alert(selected_activityId);
		if(selected_scopeType =='scopes' || selected_scopeType =='default_users' || selected_scopeType =='defaultUsers'){
			selectedUserHtml =$('#'+selected_activityId+dealType+'userName').val();
		}else if(selected_scopeType =='users' || selected_scopeType =='tree_scopes' || selected_scopeType =='orgCharge' || selected_scopeType =='orgDepart'){
			$('#select'+selected_activityId+dealType).find('option').each(function(){
				selectedUserHtml =selectedUserHtml + $(this).text() + ',';
			});
		}
		//alert(selectedUserHtml);
		if(selectedUserHtml !=null && selectedUserHtml !='' && selectedUserHtml !='null'){
			if(selectedUserHtml.length >25){
				selectedUserHtml =selectedUserHtml.substring(0,25) +'...';
			}
		}
		...
		$(obj).parent().parent().parent().find('a').each(function(){
			var styleValue =$(this).attr('class');
			if(styleValue =='a_select'){
				$(this).parent().parent().find('#selectedUser').html(selectedUserHtml);
			}
		});
	}
	...
	$(obj).parent().parent().parent().find('a').removeClass('a_select');
	$(obj).attr('class','a_select');
	
	if('<%=processInstanceId_long%>'=='0'){
		$(obj).parent().parent().parent().find('div[name=choose_subProcess_li]').removeClass('div_select');
		$(obj).parent().parent().attr('class','div_select');
	}else{
		$(obj).parent().parent().parent().find('div[name=choose_nowProcess_li]').removeClass('div_select');
		$(obj).parent().parent().attr('class','div_select');
	}
}
...
function validateForm_flow(){
	$('#wf_c_mainTable').find("input[class^='inputText'],textarea[class^='inputText'],.inputTextarea").each(function(){
...
function  checkSubmit(){
	...
	if(result){
		if(all_choosedUserName !='undefined' && all_choosedUserName.length >1000){
